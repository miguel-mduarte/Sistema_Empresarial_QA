...
function alterarColaborador(matricula, dados) {
...
test("altera um colaborador, preserva sua identidade e recalcula o salário", async () => {
  const persistidosAntes = JSON.parse(fs.readFileSync(databasePath, "utf8"));
  ...
  const persistidosDepois = JSON.parse(fs.readFileSync(databasePath, "utf8"));
